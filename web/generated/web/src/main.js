...
function currentViewerId(state) {
    return viewerForGameState(state);
}
function publicStateForViewer(state) {
...
function friendlyError(message) {
    if (message.includes("CARD_CONDITION_NOT_MET"))
        return "このカードは今の状況では使えません。";
    if (message.includes("CARD_NOT_IN_HAND"))
        return "そのカードは手札にありません。";
    if (message.includes("NOT_ACTIVE_PLAYER"))
        return "今はこのプレイヤーの手番ではありません。";
    if (message.includes("NOT_RESPONDING_PLAYER"))
        return "今は応答するプレイヤーではありません。";
    return message;
}
...
function actionCommand(commandType, playerId, payload) {
    const base = {
...
function renderCardActions(state, card, definition) {
    ...
    if (state.phase !== "ACTION_SELECTION" || state.activePlayerId !== viewerId)
        return "";
    const modes = Object.keys(definition.modes).filter((mode) => mode !== "RESPONSE");
...
function renderWorld(publicState) {
...
function renderBattle() {
    ...
    if (!publicState)
        return renderError("公開状態を作れませんでした。");
    const ownPlayer = publicState.players.find((player) => player.playerId === viewerId);
