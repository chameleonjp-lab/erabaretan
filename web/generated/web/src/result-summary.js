...
function detailsOf(event) {
    return event.details && isRecord(event.details) ? event.details : {};
}
function playerIdOf(value, playerIds) {
 ...
 */
export function collectPublicFactBatch(events, playerIds) {
    const facts = [];
    for (const event of events) {
        ...
        if (event.type === "DAMAGE_WORLD_APPLIED") {
            ...
            if (playerId && amount)
                facts.push({ type: "WORLD_DAMAGED", playerId, amount });
            continue;
        }
        if (event.type === "RESTORE_WORLD_APPLIED") {
            const amount = positiveInteger(details.effective);
            if (playerId && amount)
                facts.push({ type: "WORLD_RESTORED", playerId, amount });
            continue;
        }
        if (event.type === "WORLD_THRESHOLD_TRIGGERED") {
            const threshold = thresholdOf(details.threshold);
            if (threshold)
                facts.push({ type: "WORLD_THRESHOLD_CROSSED", thresholds: [threshold] });
            continue;
        }
        if (event.type === "DAMAGE_PLAYER_APPLIED") {
            const amount = positiveInteger(details.amount);
            if (playerId && amount)
                facts.push({ type: "PLAYER_DAMAGED", playerId, amount });
            continue;
        }
        if (event.type === "PAY_HP_APPLIED") {
            const amount = positiveInteger(details.amount);
            if (playerId && amount)
                facts.push({ type: "HP_PAID", playerId, amount });
            continue;
        }
        if (event.type === "PLAYER_DEFEATED") {
            if (playerId)
                facts.push({ type: "PLAYER_DEFEATED", playerId });
            continue;
        }
...
function candidateForLargest(batches, type, excludedBatchIndexes) {
    let best = null;
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
        if (excludedBatchIndexes.has(batchIndex))
            continue;
        const batch = batches[batchIndex];
        const amount = aggregateAmount(batch.facts, type);
        if (amount <= 0 || (best && amount <= (best.amount ?? 0)))
            continue;
        const kind = type === "WORLD_DAMAGED" ? "WORLD_DAMAGE" : "WORLD_RESTORE";
        best = {
            kind,
            batchIndex,
...
export function composeResultJudgment(summary, batches) {
    const candidates = [];
    const excludedBatchIndexes = new Set();
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
