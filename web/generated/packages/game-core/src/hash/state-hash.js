export const STATE_HASH_VERSION = "state-hash.alpha-12.v1";
...
function canonicalJsonValue(value, inArray = false) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error("state hash input cannot contain a non-finite number");
        return JSON.stringify(value);
    }
    if (value === undefined)
        return inArray ? "null" : "";
    if (typeof value !== "object")
        throw new Error(`state hash input contains unsupported value: ${typeof value}`);
    if (Array.isArray(value))
        return `[${value.map((item) => canonicalJsonValue(item, true)).join(",")}]`;
    const record = value;
...
export function projectStateForHash(state) {
    const players = {};
...
export function serializeStateForHash(state) {
