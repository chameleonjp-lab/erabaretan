...
function rejection(code, message, commandId = null) {
...
function isPlayerId(state, value) {
...
function isCommandType(value) {
    return typeof value === "string" && COMMAND_TYPES.includes(value);
}
function commandPhaseAllowed(state, commandType, playerId) {
    if (state.phase === "FINISHED")
        return rejection("MATCH_FINISHED", "FINISHED state accepts no commands", null);
    ...
    if (state.phase === "ACTION_SELECTION") {
        if (playerId !== state.activePlayerId)
            return rejection("NOT_ACTIVE_PLAYER", "only active player may act", null);
    ...
    if (state.phase === "TURN_START") {
        if (playerId !== state.activePlayerId)
            return rejection("NOT_ACTIVE_PLAYER", "only active player may resolve overflow", null);
    ...
    if (state.phase === "RESPONSE_SELECTION") {
        if (playerId !== state.respondingPlayerId)
            return rejection("NOT_RESPONDING_PLAYER", "only responding player may answer", null);
...
export function validateCommand(state, input, options = {}) {
    if (!isRecord(input))
        return rejection("MALFORMED_COMMAND", "command must be a JSON object");
    const commandId = typeof input.commandId === "string" ? input.commandId : null;
    if (!isId(input.commandId))
        return rejection("INVALID_COMMAND_ID", "commandId must be a stable non-empty identifier", commandId);
    if (!isPlayerId(state, input.playerId))
        return rejection("INVALID_PLAYER_ID", "playerId is not in this match", typeof input.commandId === "string" ? input.commandId : null);
    ...
    if (phaseError)
        return { ok: false, error: { ...phaseError.error, commandId: typeof input.commandId === "string" ? input.commandId : null } };
    ...
    if (input.commandType === "PLAY_CARD") {
        ...
        if (options.cardConditionValidator) {
            const condition = options.cardConditionValidator({
                ...
                targetPlayerId: typeof payload.targetPlayerId === "string" ? payload.targetPlayerId : null,
                discardCardInstanceId: typeof payload.discardCardInstanceId === "string" ? payload.discardCardInstanceId : null,
            });
            if (condition.ok === false)
                return rejection(condition.code, condition.message, input.commandId);
        }
        return { ok: true, kind: "NEW", command: { ...input, payload: { ...payload } } };
    }
    ...
    return { ok: true, kind: "NEW", command: { ...input, payload: {} } };
}
