...
function countReferences(groups) {
...
export function assertGameState(state) {
    ...
    if (state.initialPlayerOrder.length !== 2)
        throw new Error("initialPlayerOrder must contain two players");
    if (new Set(state.initialPlayerOrder).size !== 2)
        throw new Error("initialPlayerOrder must be unique");
    ...
    if (references.size !== cardIds.length)
        throw new Error("every card instance must be in exactly one zone");
    ...
    for (const cardId of cardIds) {
        if (references.get(cardId) !== 1)
            throw new Error(`card ${cardId} appears in multiple or no zones`);
    }
    for (const [playerId, player] of Object.entries(state.players)) {
        if (player.playerId !== playerId)
            throw new Error(`player key mismatch: ${playerId}`);
        ...
        if (!hand)
            throw new Error(`missing cardZones hand for ${playerId}`);
    ...
    for (const [cardId, card] of Object.entries(state.cardInstances)) {
        if (card.cardInstanceId !== cardId)
            throw new Error(`card key mismatch: ${cardId}`);
        if (!state.players[card.ownerPlayerId])
            throw new Error(`card ${cardId} has unknown owner`);
        assertIntegerInRange(`${cardId}.drawOrder`, card.drawOrder, 0, Number.MAX_SAFE_INTEGER);
        const actualZone = findCardZone(state, cardId);
        if (actualZone !== card.zone)
            throw new Error(`card ${cardId} zone mismatch: ${card.zone} vs ${actualZone}`);
    ...
    if (state.activeField !== null) {
        if (!state.players[state.activeField.ownerPlayerId])
            throw new Error("activeField owner is unknown");
    ...
    for (const modifier of state.scoreModifiers) {
        if (!state.players[modifier.playerId])
            throw new Error("score modifier contains an unknown player");
    ...
    if (state.phase === "RESPONSE_SELECTION") {
        ...
        if (!state.respondingPlayerId)
            throw new Error("RESPONSE_SELECTION requires respondingPlayerId");
        if (!state.pendingAttack)
            throw new Error("RESPONSE_SELECTION requires pendingAttack");
        ...
        if (state.pendingAttack.pendingAttackId !== state.pendingAction.pendingAttackId
...
export function findCardZone(state, cardInstanceId) {
    if (state.cardZones.drawPile.includes(cardInstanceId))
        return "DRAW_PILE";
    if (state.cardZones.discardPile.includes(cardInstanceId))
        return "DISCARD_PILE";
