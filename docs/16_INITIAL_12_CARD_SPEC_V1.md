...
- 終端検証：[ゴールデン試合と最終状態ハッシュ V1](21_GOLDEN_MATCHES_AND_STATE_HASHES_V1.md)
...
P0-06で、通常撃破、世界崩壊、反射を含む同時終了のゴールデン試験と最終状態ハッシュを[終端検証正本](21_GOLDEN_MATCHES_AND_STATE_HASHES_V1.md)へ固定した。

- [x] 75、50、25の世界律の具体的な効果をP0-02で定義した
- [x] 初期`ruleset`の全数値と採点係数をP0-02で定義した
- 防御値、盾、世界損傷が同じ解決内で混ざる場合の全処理例
- [x] 決定的シャッフルのアルゴリズムと固定試験値をP0-05で定義した
- [x] Fixture A〜F、X01〜X14の完全な初期状態と期待結果をP0-03で定義した
- [x] 共通効果命令の正式型と初期12種類の命令展開をP0-04で定義した
- 最終版のカード数値、使用率、勝率

P0-01からP0-06の正本が揃った。次はP1-01の`game-core`基盤実装へ進む。
...
- [x] 決定的乱数と初期山札の固定試験値をP0-05の正本へ定義した。
- [x] ゴールデン試合3件と最終状態ハッシュをP0-06の正本へ定義した。

この文書でP0-01を完了した。P0-02〜P0-06の正本を参照し、次はP1-01「純粋なstate / command基盤」へ進む。
