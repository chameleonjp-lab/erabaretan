...
import type { InitialCardDefinition } from "../../packages/content/src/index.ts";
import {
  projectPublicState,
  summarizeMatch,
...
import { applyLocalCommand, createLocalMatch } from "./local-match.ts";
...
function previewLabel(state: GameState, intent: PreviewCommandIntent, result: PreviewResult): string {
  ...
  if (result.delta.worldDurabilityDelta < 0) parts.push(`世界へ${Math.abs(result.delta.worldDurabilityDelta)}`);
  if (result.delta.worldDurabilityDelta > 0) parts.push(`世界へ${result.delta.worldDurabilityDelta}回復`);
...
function renderPlayers(publicState: PublicGameState): string {
