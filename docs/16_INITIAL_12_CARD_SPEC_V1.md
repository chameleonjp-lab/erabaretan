...
P0-01では次を決めない。P0-02またはP0-03で固定する。

- 75、50、25の世界律の具体的な効果
- 初期`ruleset`の全数値と採点係数
- 防御値、盾、世界損傷が同じ解決内で混ざる場合の全処理例
- 決定的シャッフルのアルゴリズムと固定試験値
- Fixture A〜F、X01〜X14の完全な初期状態と期待結果
- 最終版のカード数値、使用率、勝率

P0-02とP0-03が終わるまで、`game-core`本実装、画面、CPU、通信は開始しない。
...
- [ ] 最初の世界律を定義する。
- [ ] `ruleset`検証値を一か所へ固定する。
- [ ] Fixture A〜F、X01〜X14を仕様化する。

この文書の追加でP0-01を完了とし、次はP0-02「最初の世界律と`ruleset`検証値」へ進む。
