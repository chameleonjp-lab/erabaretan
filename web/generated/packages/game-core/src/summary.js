import { calculateJudgmentBreakdown } from "./judgment/calculate.js";
import { assertGameState } from "./state/invariants.js";
export const MATCH_SUMMARY_VERSION = "match-summary.alpha-12.v1";
function normalEndReasons(state) {
    const reasons = [];
    if (state.terminalFlags.defeatedPlayerIds.length > 0)
        reasons.push("PLAYER_DEFEATED");
    if (state.terminalFlags.worldCollapsed)
        reasons.push("WORLD_COLLAPSED");
    if (state.terminalFlags.maxRoundsReached)
        reasons.push("MAX_ROUNDS_REACHED");
    return reasons;
}
function judgmentMatches(state) {
    if (!state.judgment)
        return false;
    ...
    if (savedIds.length !== expectedIds.length || savedIds.some((playerId) => !expectedIds.includes(playerId)))
        return false;
    if (calculated.winnerId !== state.judgment.winnerId)
        return false;
    if (state.terminalFlags.divineSelectionWinnerId !== state.judgment.winnerId)
        return false;
    return expectedIds.every((playerId) => calculated.playerScores[playerId] === state.judgment?.playerScores[playerId]);
}
function hasOnlyKnownPlayerIds(state, playerIds) {
    return new Set(playerIds).size === playerIds.length
        && playerIds.every((playerId) => state.initialPlayerOrder.includes(playerId));
}
function terminalStateConsistent(state) {
    const flags = state.terminalFlags;
    if (!hasOnlyKnownPlayerIds(state, flags.defeatedPlayerIds))
        return false;
    if (flags.battleWinnerId !== null && !state.initialPlayerOrder.includes(flags.battleWinnerId))
        return false;
    if (flags.divineSelectionWinnerId !== null && !state.initialPlayerOrder.includes(flags.divineSelectionWinnerId))
        return false;
    if (state.activePlayerId !== null || state.respondingPlayerId !== null)
        return false;
    if (state.pendingAction !== null || state.pendingAttack !== null || state.effectQueue.length !== 0)
        return false;
    if (flags.worldCollapsed !== (state.world.durability === 0))
        return false;
    if (flags.maxRoundsReached !== (state.roundNumber > state.ruleset.maxRounds))
        return false;
    ...
    if (state.judgment !== null || flags.divineSelectionWinnerId !== null)
        return false;
    if (flags.endKind === "SURRENDER" || flags.endKind === "DISCONNECT_FORFEIT") {
        if (flags.defeatedPlayerIds.length !== 1 || flags.battleWinnerId === null)
            return false;
...
function playerSummaries(state, breakdown) {
...
function summaryFor(state, breakdown) {
    ...
    const divineSelection = endKind !== "NORMAL"
...
export function summarizeMatch(state) {
    ...
    if (!terminalStateConsistent(state))
        return { ok: false, code: "TERMINAL_STATE_INCONSISTENT" };
    const endKind = state.terminalFlags.endKind;
    if (endKind === null)
        return { ok: false, code: "TERMINAL_STATE_INCONSISTENT" };
    if (endKind === "NORMAL") {
        if (normalEndReasons(state).length === 0)
            return { ok: false, code: "TERMINAL_STATE_INCONSISTENT" };
        if (!state.judgment)
            return { ok: false, code: "JUDGMENT_MISSING" };
        try {
            if (!judgmentMatches(state))
