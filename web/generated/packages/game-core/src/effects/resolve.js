...
import { finalizeTerminalState } from "../terminal/resolve.js";
...
function result(effect, status, values = {}) {
    return {
...
function event(effect, type, details = {}) {
    return { type, effectId: effect.effectId, commandType: effect.commandType, details };
}
function reject(effect, code, values = {}) {
    return { state: null, result: result(effect, "REJECTED", { before: values, rejectionCode: code }), events: [] };
}
function targetPlayerId(target) {
    if (target.targetKind === "PLAYER" || target.targetKind === "SELF_HAND")
...
function updatePlayerAndHand(state, playerId, hand, updateCardInstances) {
...
function validateEffectCommand(state, input) {
    if (!isRecord(input))
        return { ok: false, code: "EFFECT_STATE_INCONSISTENT" };
    if (typeof input.effectId !== "string" || !EFFECT_ID_PATTERN.test(input.effectId)) {
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    }
    if (typeof input.commandType !== "string" || !EFFECT_TYPES.includes(input.commandType)) {
    ...
    if (typeof target.targetKind !== "string")
    ...
    if ((type === "DAMAGE_WORLD" || type === "RESTORE_WORLD") && target.targetKind !== "WORLD")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "REDUCE_INCOMING_DAMAGE" && target.targetKind !== "CURRENT_PENDING_ATTACK")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if ((type === "DRAW_CARD" || type === "DISCARD_CARD") && !["PLAYER", "SELF_HAND"].includes(target.targetKind))
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if ((type === "DRAW_CARD" || type === "DISCARD_CARD") && !state.players[String(target.playerId)])
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if ((type === "SET_FIELD" || type === "CLEAR_FIELD") && target.targetKind !== "CURRENT_FIELD")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "REVEAL_PUBLIC_INFORMATION" && target.targetKind !== "PUBLIC_INFORMATION")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "ADD_SCORE_MODIFIER" && target.targetKind !== "SCORE_LEDGER")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "CLEAR_FIELD" && target.targetKind !== "CURRENT_FIELD")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    ...
    if (type === "DRAW_CARD" && !isInteger(payload.count, 1, 9))
        return { ok: false, code: "EFFECT_OUT_OF_RANGE" };
    ...
    if (type === "HEAL_PLAYER" && payload.allowRevive !== false)
        return { ok: false, code: "EFFECT_REVIVE_FORBIDDEN" };
    if (type === "PAY_HP" && source.ownerPlayerId !== target.playerId)
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "REFLECT_DAMAGE" && (target.targetKind !== "PLAYER" || typeof payload.pendingAttackId !== "string"))
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "ADD_SHIELD" && payload.scope === "CURRENT_PENDING_ATTACK" && target.targetKind !== "CURRENT_PENDING_ATTACK" && typeof payload.pendingAttackId !== "string")
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "ADD_SHIELD" && ["NEXT_APPLICABLE_ATTACK", "UNTIL_TURN_SEQUENCE"].includes(String(payload.scope)) && !isInteger(payload.expiresAfterTurnSequence, 1, Number.MAX_SAFE_INTEGER))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "ADD_SHIELD" && target.targetKind === "CURRENT_PENDING_ATTACK" && typeof target.pendingAttackId !== "string")
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "REDUCE_INCOMING_DAMAGE" && typeof target.pendingAttackId !== "string")
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "REFLECT_DAMAGE" && typeof target.playerId !== "string")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "DAMAGE_PLAYER" && !["DIRECT", "REFLECTION", "FRAGILE_WORLD", "WORLD_LAW"].includes(String(payload.damageKind)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "ADD_SHIELD" && !["CURRENT_PENDING_ATTACK", "NEXT_APPLICABLE_ATTACK", "UNTIL_TURN_SEQUENCE"].includes(String(payload.scope)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "ADD_SHIELD" && payload.scope !== "CURRENT_PENDING_ATTACK" && target.targetKind !== "PLAYER")
        return { ok: false, code: "EFFECT_BAD_TARGET" };
    if (type === "DRAW_CARD" && !["NORMAL_REFILL", "WORLD_LAW_50", "CARD_EFFECT"].includes(String(payload.reason)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "DISCARD_CARD" && (![
        "EXPLICIT_CARD_INSTANCE",
        "NEWEST_CARD_INSTANCE",
        "OLDEST_CARD_INSTANCE",
    ].includes(String(payload.selection?.selectionKind)) || !["CARD_EFFECT", "OVERFLOW", "TIMEOUT"].includes(String(payload.reason))))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if ((type === "DAMAGE_WORLD" || type === "RESTORE_WORLD") && !["CARD_RELEASE", "CARD_RESPONSE", "FIELD_EFFECT", "WORLD_LAW"].includes(String(payload.reason)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "CLEAR_FIELD" && !["CARD_RELEASE", "CARD_EFFECT", "WORLD_LAW"].includes(String(payload.reason)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "MODIFY_STAT_UNTIL_TURN_END" && !["INCOMING_DAMAGE_REDUCTION", "ACTION_DAMAGE"].includes(String(payload.stat)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "MODIFY_NEXT_ACTION" && (![
        "DEFENSE_VALUE",
        "INCOMING_DAMAGE_REDUCTION",
        "ACTION_DAMAGE",
    ].includes(String(payload.stat)) || !["NEXT_DEFENSE_CARD", "NEXT_APPLICABLE_ATTACK", "NEXT_ACTION"].includes(String(payload.consumeWhen))))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (type === "REVEAL_PUBLIC_INFORMATION" && (![
        ...
        "DRAW_OCCURRED",
        "MATCH_FINISHED",
    ].includes(String(payload.informationKind)) || (payload.threshold !== undefined && !isInteger(payload.threshold, 0, 100))))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    ...
    ].includes(String(payload.modifierKind)))
        return { ok: false, code: "EFFECT_MISSING_FIELD" };
    if (state.phase === "FINISHED" || state.phase === "JUDGMENT")
        return { ok: false, code: "EFFECT_CONDITION_NOT_MET" };
    return { ok: true, effect: input };
}
function applyDamage(state, effect, payload, bypassDefense) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId)
        return reject(effect, "EFFECT_BAD_TARGET");
    const player = state.players[playerId];
    if (!player || player.hitPoints <= 0)
        return reject(effect, "EFFECT_CONDITION_NOT_MET", { hitPoints: player?.hitPoints ?? null });
    ...
    if (!bypassDefense && (effect.executionTiming === "AFTER_RESPONSE_MODIFIERS" || fragileWorldDamage)) {
        if (effect.executionTiming === "AFTER_RESPONSE_MODIFIERS") {
            if (!pending || pending.defendingPlayerId !== playerId)
                return reject(effect, "EFFECT_BAD_TARGET");
            applicableShields = player.statusEffects.shields.filter((shield) => {
                if (shield.scope === "NEXT_APPLICABLE_ATTACK")
            ...
            const nextStatus = {
                ...
                shields: player.statusEffects.shields.filter((shield) => {
                    if (consumedShields.includes(shield))
                        return false;
                    if (shield.scope !== "CURRENT_PENDING_ATTACK" && (shield.expiresAfterTurnSequence ?? Number.MAX_SAFE_INTEGER) <= state.turnSequence)
                        return false;
                    return true;
                }),
            };
    ...
    if (updatedPlayer.hitPoints > 0 && updatedPlayer.hitPoints - effective === 0)
        eventTypes.push("PLAYER_DEFEATED");
    return {
        state: nextState,
...
function applyPayHp(state, effect, payload) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId)
        return reject(effect, "EFFECT_BAD_TARGET");
    const player = state.players[playerId];
...
function applyAddShield(state, effect, payload) {
...
function applyReduceIncomingDamage(state, effect, payload) {
    if (!state.pendingAttack || effect.target.targetKind !== "CURRENT_PENDING_ATTACK" || effect.target.pendingAttackId !== state.pendingAttack.pendingAttackId)
        return reject(effect, "EFFECT_PENDING_ATTACK_REQUIRED");
...
function applyDrawCard(state, effect, payload) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId)
        return reject(effect, "EFFECT_BAD_TARGET");
    const player = state.players[playerId];
    ...
    if (count <= 0)
        return { state, result: result(effect, "NO_OP", { requested: payload.count, effective: 0, before: { handCount: player.hand.length, drawPileCount: state.cardZones.drawPile.length } }), events: [] };
    ...
    for (const cardId of drawn)
        nextCards[cardId] = { ...nextCards[cardId], ownerPlayerId: playerId, zone: "HAND" };
...
function applyDiscardCard(state, effect, payload) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId)
        return reject(effect, "EFFECT_BAD_TARGET");
    const player = state.players[playerId];
    let cardId;
    if (payload.selection.selectionKind === "EXPLICIT_CARD_INSTANCE")
        cardId = payload.selection.cardInstanceId;
    if (payload.selection.selectionKind === "NEWEST_CARD_INSTANCE")
        cardId = player.hand[player.hand.length - 1];
    if (payload.selection.selectionKind === "OLDEST_CARD_INSTANCE")
        cardId = player.hand[0];
    if (!cardId || !player.hand.includes(cardId) || state.cardZones.inResolution.includes(cardId))
        return reject(effect, "EFFECT_CONDITION_NOT_MET");
    const hand = player.hand.filter((id) => id !== cardId);
    const nextCards = { ...state.cardInstances, [cardId]: { ...state.cardInstances[cardId], zone: "DISCARD_PILE" } };
...
function applyDamageWorld(state, effect, payload) {
    ...
    if (recordsPlayerLedger && !owner)
        return reject(effect, "EFFECT_BAD_SOURCE");
    ...
    for (const threshold of state.ruleset.worldThresholds) {
        if (beforeDurability > threshold && afterDurability <= threshold && !triggered.includes(threshold))
...
function applyRestoreWorld(state, effect, payload) {
    const owner = effect.source.ownerPlayerId;
    const recordsPlayerLedger = effect.attributionPolicy === "SOURCE_OWNER" || effect.attributionPolicy === "ORIGINAL_CARD_OWNER";
    if (recordsPlayerLedger && !owner)
        return reject(effect, "EFFECT_BAD_SOURCE");
    ...
    const worldState = { ...state, world: { ...state.world, durability: beforeDurability + effective } };
    const nextState = recordsPlayerLedger && owner
...
function applySetField(state, effect, payload) {
    if (payload.expiresAfterTurnSequence <= state.turnSequence || payload.ownerPlayerId !== effect.source.ownerPlayerId)
        return reject(effect, "EFFECT_CONDITION_NOT_MET");
...
function applyClearField(state, effect) {
    if (!state.activeField)
        return reject(effect, "EFFECT_NO_ACTIVE_FIELD");
...
function applyModifyStat(state, effect, payload) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId || payload.expiresAfterTurnSequence <= state.turnSequence || payload.delta === 0)
        return reject(effect, "EFFECT_CONDITION_NOT_MET");
    const modifier = { stat: payload.stat, delta: payload.delta, expiresAfterTurnSequence: payload.expiresAfterTurnSequence };
...
function applyModifyNextAction(state, effect, payload) {
    const playerId = targetPlayerId(effect.target);
    if (!playerId || payload.delta === 0)
        return reject(effect, "EFFECT_CONDITION_NOT_MET");
    ...
    const modifier = { stat: payload.stat === "ACTION_DAMAGE" ? "ACTION_DAMAGE" : "INCOMING_DAMAGE_REDUCTION", delta: payload.delta, expiresAfterTurnSequence: state.turnSequence + 1 };
    const nextState = updatePlayer(state, playerId, (player) => ({ ...player, statusEffects: { ...player.statusEffects, statModifiers: [...player.statusEffects.statModifiers, modifier] } }));
    return {
        state: nextState,
...
function applyScoreModifier(state, effect, payload) {
    const modifier = { playerId: payload.playerId, modifierKind: payload.modifierKind, amount: payload.amount };
...
export function beginPendingAttack(state, pending) {
    if (state.phase !== "RESOLUTION" && state.phase !== "RESPONSE_SELECTION")
        throw new Error("pending attack must be opened during resolution");
    const nextState = {
        ...state,
        pendingAttack: {
            ...pending,
...
export function applyEffect(state, input, usedEffectIds = new Set()) {
    const validation = validateEffectCommand(state, input);
    if (validation.ok === false) {
        const effect = (isRecord(input) ? input : { effectId: "effect.invalid.0000", commandType: "DAMAGE_PLAYER" });
        return { state, result: result(effect, "REJECTED", { rejectionCode: validation.code }), events: [] };
    }
    const effect = validation.effect;
    if (usedEffectIds.has(effect.effectId))
        return { state, result: result(effect, "REJECTED", { rejectionCode: "EFFECT_DUPLICATE_ID" }), events: [] };
    let transition;
    switch (effect.commandType) {
        case "DAMAGE_PLAYER":
            transition = applyDamage(state, effect, effect.payload, effect.payload.damageKind === "REFLECTION");
            break;
        case "HEAL_PLAYER": {
            const playerId = targetPlayerId(effect.target);
            const payload = effect.payload;
            if (!playerId || state.players[playerId].hitPoints <= 0)
                transition = reject(effect, "EFFECT_REVIVE_FORBIDDEN");
            else {
                const player = state.players[playerId];
                const effective = Math.min(payload.amount, player.maxHitPoints - player.hitPoints);
                const nextState = updatePlayer(state, playerId, (current) => ({ ...current, hitPoints: current.hitPoints + effective }));
                transition = { state: nextState, result: result(effect, effective === 0 ? "NO_OP" : "APPLIED", { requested: payload.amount, effective, before: { hitPoints: player.hitPoints }, after: { hitPoints: player.hitPoints + effective }, eventTypes: effective ? ["HEAL_PLAYER_APPLIED"] : [] }), events: effective ? [event(effect, "HEAL_PLAYER_APPLIED", { playerId, amount: effective })] : [] };
            }
            break;
        }
        case "PAY_HP":
            transition = applyPayHp(state, effect, effect.payload);
            break;
        case "ADD_SHIELD":
            transition = applyAddShield(state, effect, effect.payload);
            break;
        case "REDUCE_INCOMING_DAMAGE":
            transition = applyReduceIncomingDamage(state, effect, effect.payload);
            break;
        case "REFLECT_DAMAGE": {
            const payload = effect.payload;
            if (!state.pendingAttack || state.pendingAttack.pendingAttackId !== payload.pendingAttackId || state.pendingAttack.reflectionApplied || effect.target.targetKind !== "PLAYER" || effect.target.playerId !== state.pendingAttack.attackingPlayerId)
                transition = reject(effect, "EFFECT_CONDITION_NOT_MET");
            else {
                const amount = Math.min(payload.amount, state.pendingAttack.effectiveDamage ?? state.pendingAttack.baseDamage);
                const reflected = applyDamage(state, { ...effect, payload: { amount, damageKind: "REFLECTION" }, executionTiming: "IMMEDIATE" }, { amount, damageKind: "REFLECTION" }, true);
                if (!reflected.state)
                    transition = reflected;
                else {
                    transition = { ...reflected, state: { ...reflected.state, pendingAttack: { ...state.pendingAttack, reflectionApplied: true } }, result: { ...reflected.result, commandType: "REFLECT_DAMAGE", eventTypes: ["REFLECT_DAMAGE_APPLIED", ...reflected.result.eventTypes] }, events: [event(effect, "REFLECT_DAMAGE_APPLIED", { playerId: effect.target.playerId, amount }), ...reflected.events] };
                }
            }
            break;
        }
        case "DRAW_CARD":
            transition = applyDrawCard(state, effect, effect.payload);
            break;
        case "DISCARD_CARD":
            transition = applyDiscardCard(state, effect, effect.payload);
            break;
        case "DAMAGE_WORLD":
            transition = applyDamageWorld(state, effect, effect.payload);
            break;
        case "RESTORE_WORLD":
            transition = applyRestoreWorld(state, effect, effect.payload);
            break;
        case "SET_FIELD":
            transition = applySetField(state, effect, effect.payload);
            break;
        case "CLEAR_FIELD":
            transition = applyClearField(state, effect);
            break;
        case "MODIFY_STAT_UNTIL_TURN_END":
            transition = applyModifyStat(state, effect, effect.payload);
            break;
        case "MODIFY_NEXT_ACTION":
            transition = applyModifyNextAction(state, effect, effect.payload);
            break;
        case "REVEAL_PUBLIC_INFORMATION":
            transition = { state, result: result(effect, "APPLIED", { requested: 1, effective: 1, eventTypes: ["PUBLIC_INFORMATION_REVEALED"] }), events: [event(effect, "PUBLIC_INFORMATION_REVEALED", {})] };
            break;
        case "ADD_SCORE_MODIFIER":
            transition = applyScoreModifier(state, effect, effect.payload);
            break;
    }
    if (transition.state && (transition.result.status === "APPLIED" || transition.result.status === "NO_OP"))
        assertGameState(transition.state);
    return transition.state ? transition : { ...transition, state };
}
function invalidMatchState(state) {
    return {
        ...state,
        phase: "FINISHED",
        activePlayerId: null,
...
function annotateWorldLawTransition(transition, effect, details) {
    const retainedEventTypes = transition.result.eventTypes.filter((type) => (type !== "PUBLIC_INFORMATION_REVEALED"
        && type !== "NEXT_ACTION_MODIFIED"
        && type !== "WORLD_LAW_EFFECT_APPLIED"));
    const retainedEvents = transition.events.filter((currentEvent) => (currentEvent.type !== "PUBLIC_INFORMATION_REVEALED"
...
function activateFragileWorld(state) {
...
export function resolveEffectQueue(state, effects) {
    ...
    const newlyTriggeredThresholds = new Set();
    const cardsCrossing25 = new Set();
    const fragileDamageCards = new Set();
    let pendingFragileSelfDamage = null;
    let lastEffectiveRestorePlayerId = null;
    let index = 0;
    while (index < queue.length || pendingFragileSelfDamage) {
        ...
        for (const threshold of transition.state.world.triggeredThresholds) {
            if (!beforeThresholds.has(threshold))
                newlyTriggeredThresholds.add(threshold);
        }
        if (currentEffect.commandType === "RESTORE_WORLD"
    ...
    const applyGenerated = (generatedEffect, details, stateTransform = (currentState) => currentState) => {
        if (effectBudgetUsed >= state.ruleset.maxEffectsPerResolution)
            return { ok: false, rejectionCode: "EFFECT_QUEUE_LIMIT" };
        effectBudgetUsed += 1;
        ...
        if (!generatedState)
            return { ok: false, rejectionCode: "EFFECT_STATE_INCONSISTENT" };
    ...
    for (const threshold of state.ruleset.worldThresholds) {
        if (!newlyTriggeredThresholds.has(threshold))
            continue;
        if (threshold === 75) {
            ...
            if (targets.length === 0) {
                ...
                if (applied.ok === false)
                    return { committed: false, state: invalidMatchState(state), results, events: [], rejectionCode: applied.rejectionCode };
            }
            else {
    ...
    if (working !== beforeTerminal) {
        ...
        if (working.judgment)
            events.push(event(terminalEffect, "JUDGMENT_COMPUTED", { winnerId: working.judgment.winnerId }));
