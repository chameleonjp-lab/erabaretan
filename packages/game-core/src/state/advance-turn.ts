 ...
 */
export function advanceToNextTurnStart(state: GameState): GameState {
  if (state.phase !== "TURN_END") throw new Error("next turn can only begin from TURN_END");
  if (!state.activePlayerId) throw new Error("TURN_END requires an active player");
  const currentIndex = state.initialPlayerOrder.indexOf(state.activePlayerId);
  if (currentIndex < 0) throw new Error("active player is not in initialPlayerOrder");
 ...
 */
export function resolveTurnStart(state: GameState): GameState {
  if (state.phase !== "TURN_START") throw new Error("turn start can only be resolved from TURN_START");
  if (!state.activePlayerId) throw new Error("TURN_START requires an active player");

  const playerId = state.activePlayerId;
  const player = state.players[playerId];
  if (!player) throw new Error(`Unknown active player: ${playerId}`);
  ...
  const nextCardInstances = drawCardId
    ? {
        ...state.cardInstances,
        [drawCardId]: { ...state.cardInstances[drawCardId], ownerPlayerId: playerId, zone: "HAND" as const },
      }
    : state.cardInstances;
